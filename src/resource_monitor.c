...
void resource_monitor_get_telemetry(telemetry_packet_t *telemetry)
{
    ...
    telemetry->heap_free_kb = htons(free_kb);
...
static int send_telemetry(void)
{
    ...
    resource_monitor_get_telemetry(&packet);

    int ret = zsock_sendto(telemetry_sock, &packet, sizeof(packet), 0,
                           (struct sockaddr *)&topside_addr, sizeof(topside_addr));
