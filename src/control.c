...
/* Manipulator output (TIM4_CH4 / PD15 / D9). */
static const struct pwm_dt_spec manipulator_pwm = PWM_DT_SPEC_GET(DT_NODELABEL(rov_manipulator_pwm));
...
#define MANIP_MIN_PULSE_US      1000U
#define MANIP_NEUTRAL_PULSE_US  1500U
#define MANIP_MAX_PULSE_US      2000U
#define MANIP_MAX_DEG           50.0f
#define MANIP_SLEW_US           50U
...
static uint16_t manipulator_applied_us = MANIP_NEUTRAL_PULSE_US;
...
{
    int32_t pulse = (int32_t)MANIP_NEUTRAL_PULSE_US +
                    ((int32_t)command *
                     ((int32_t)MANIP_MAX_PULSE_US - (int32_t)MANIP_NEUTRAL_PULSE_US)) / 127;

    if (pulse < (int32_t)MANIP_MIN_PULSE_US) {
        return MANIP_MIN_PULSE_US;
    }
    if (pulse > (int32_t)MANIP_MAX_PULSE_US) {
        return MANIP_MAX_PULSE_US;
    }
...
{
    return ((float)pulse_us - (float)MANIP_NEUTRAL_PULSE_US) *
           (MANIP_MAX_DEG / ((float)MANIP_MAX_PULSE_US - (float)MANIP_NEUTRAL_PULSE_US));
}
...
static void rov_set_manipulator(int8_t command)
{
    ...
    if (target_us > manipulator_applied_us + MANIP_SLEW_US) {
        manipulator_applied_us += MANIP_SLEW_US;
    } else if (target_us + MANIP_SLEW_US < manipulator_applied_us) {
        manipulator_applied_us -= MANIP_SLEW_US;
    } else {
    ...
    if (pwm_is_ready_dt(&manipulator_pwm)) {
        ...
        if (ret < 0) {
            LOG_ERR("Failed to set manipulator PWM (%u us): %d",
                    (unsigned int)manipulator_applied_us, ret);
        }
    }
...
void rov_control_init(void)
{
    ...
    if (!pwm_is_ready_dt(&manipulator_pwm)) {
        ...
        manipulator_applied_us = MANIP_NEUTRAL_PULSE_US;
        if (pwm_set_pulse_dt(&manipulator_pwm, MANIP_NEUTRAL_PULSE_US * 1000U) < 0) {
    ...
    ctrl_telem.manipulator_pulse_us = MANIP_NEUTRAL_PULSE_US;
    k_mutex_unlock(&ctrl_telem_mutex);
...
void rov_send_command(uint32_t sequence, uint64_t payload)
{
    ...
    command.manipulator = (int8_t)((int16_t)((payload >> 56) & 0xFF) - 128);
