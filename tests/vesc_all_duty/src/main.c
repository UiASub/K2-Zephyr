...
#define FIRST_CAN_ID 1
#define LAST_CAN_ID 7
...
static void send_local_duty(float duty)
{
    uint8_t packet[32];
...
static void send_can_duty(uint8_t can_id, float duty)
{
    uint8_t packet[32];
    const size_t len = vesc_build_set_duty_can(packet, can_id, duty);

    uart_send(packet, len);
}

static void send_all_thrusters(float duty)
{
    send_local_duty(duty);

    for (uint8_t can_id = FIRST_CAN_ID; can_id <= LAST_CAN_ID; can_id++) {
        send_can_duty(can_id, duty);
    }
}

int main(void)
{
    printk("\nK2 VESC all-thruster duty test\n");
    printk("UART: %s, %d 8N1\n", vesc_uart->name, BAUDRATE);
    printk("PB6/D1 TX -> local VESC RX, PB7/D0 RX -> local VESC TX, common GND.\n");
    printk("Local UART VESC + CAN IDs %d..%d duty: %.2f\n",
           FIRST_CAN_ID, LAST_CAN_ID, (double)DUTY_CYCLE);
    ...
    printk("Sending zero duty to all thrusters for startup delay...\n");
    for (int i = 0; i < START_DELAY_MS / COMMAND_PERIOD_MS; i++) {
        send_all_thrusters(0.0f);
        k_sleep(K_MSEC(COMMAND_PERIOD_MS));
    }

    printk("Sending duty %.2f to all thrusters every %d ms.\n",
           (double)DUTY_CYCLE, COMMAND_PERIOD_MS);

    while (true) {
        send_all_thrusters(DUTY_CYCLE);
        k_sleep(K_MSEC(COMMAND_PERIOD_MS));
    }
}
